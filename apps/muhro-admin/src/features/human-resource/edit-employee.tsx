...
import { editEmployeeValidationSchema } from "@/validations/employee.schema";
...
const EditEmployee = () => {
  ...
  const employeeDataResponse = useFetchData(
    [queryKeys.get_employees, id],
    `${queryKeys.get_employees}/${id}`
  );

  const employeeData: EmployeeDataProps = employeeDataResponse?.data?.data;
  const employeeName = employeeData?.name ? employeeData.name.split(" ") : [];
  ...
  const initialValues: EmployeeFormValues = {
    ...
    role: employeeData?.roles.map((role) => role.uuid) || [],
    profileImage: null,
  };

  const handleFormSubmission = async (values: EmployeeFormValues) => {
    try {
      ...
      await postMutation.mutateAsync({
        url: `${queryKeys.get_employees}/${id}`,
        method: "PATCH",
        body: formData,
            ...
            <div className="grid grid-cols-12 gap-4 p-2 justify-between w-full">
              ...
              <div className="col-span-12 md:col-span-8 ">
                <FileUploader
                  ...
                  previewUrl={employeeData.profile_photo_url}
                />
